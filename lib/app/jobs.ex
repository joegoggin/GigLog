defmodule App.Jobs do
  ...
  def subscribe_jobs(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(App.PubSub, "user:#{key}:jobs")
  end

  defp broadcast_job(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(App.PubSub, "user:#{key}:jobs", message)
  end
  ...
  def create_job(%Scope{} = scope, attrs) do
    with {:ok, job = %Job{}} <-
           ...
           |> Repo.insert() do
      broadcast_job(scope, {:created, job})
      {:ok, job}
    end
  end
  ...
  def update_job(%Scope{} = scope, %Job{} = job, attrs) do
    ...
    with {:ok, job = %Job{}} <-
           ...
           |> Job.changeset(attrs, scope)
           |> Repo.update() do
      broadcast_job(scope, {:updated, job})
      {:ok, job}
    end
  end
  ...
  def delete_job(%Scope{} = scope, %Job{} = job) do
    ...
    with {:ok, job = %Job{}} <-
           Repo.delete(job) do
      broadcast_job(scope, {:deleted, job})
      {:ok, job}
    end
  end
